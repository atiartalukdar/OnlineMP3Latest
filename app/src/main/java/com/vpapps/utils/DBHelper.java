    ...
    public String getRecentNewsIDs(String limit) {
        ...
        if (cursor != null && cursor.getCount() > 0) {
            ...
            for (int i = 1; i < cursor.getCount(); i++) {
                newsIDs = newsIDs + "," + cursor.getString(cursor.getColumnIndex(TAG_SID));
    ...
    public ArrayList<ItemSong> loadDataRecent(Boolean isOnline, String limit) {
        ...
        if (cursor != null && cursor.getCount() > 0) {
            ...
            for (int i = 0; i < cursor.getCount(); i++) {
                ...
                ItemSong objItem = new ItemSong(id, cid, cname, artist, url, imagebig, imagesmall, name, desc, total_rate, avg_rate, views, downloads, false);
                arrayList.add(objItem);
